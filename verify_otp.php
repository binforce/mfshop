...
if (isset($_POST['verify_otp'])) {
    ...
    if ($user['otp_code'] == $otp_input && $user['otp_expiry'] >= $current_time) {
        $_SESSION['user_id'] = $user_id; $_SESSION['role'] = $user['role'];
        $conn->query("UPDATE users SET otp_code=NULL, otp_expiry=NULL WHERE id=$user_id");
        $_SESSION['sys_msg'] = ['type' => 'success', 'title' => 'Hoàn tất', 'msg' => 'Xác thực tài khoản thành công!'];
        header("Location: " . ($user['role'] == 0 ? "admin.php" : "index.php")); exit();
    } else {
        $sys_msg = ['type' => 'error', 'title' => 'Thất bại', 'msg' => 'Mã OTP không đúng hoặc đã hết hạn!'];
    }
}
?>
<!DOCTYPE html>
