...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $username = trim($_POST['username']); $password = md5($_POST['password']); 
    $email = trim($_POST['email']); $name = trim($_POST['name']);
    $phone = trim($_POST['phone']); $address = trim($_POST['address']);
    $dob = $_POST['dob']; $gender = (int)$_POST['gender'];

    $check_stmt = $conn->prepare("SELECT id FROM users WHERE username = ? OR email = ?");
    $check_stmt->bind_param("ss", $username, $email); $check_stmt->execute();
    if ($check_stmt->get_result()->num_rows > 0) {
        $sys_msg = ['type' => 'error', 'title' => 'Trùng lặp', 'msg' => 'Tên đăng nhập hoặc Email đã tồn tại!'];
    } else {
        $otp = sprintf("%06d", mt_rand(1, 999999)); $expiry = date("Y-m-d H:i:s", strtotime('+5 minutes'));
        ...
        $stmt->bind_param("sssssssiiss", $username, $password, $email, $name, $phone, $address, $dob, $gender, $otp, $expiry);
        if ($stmt->execute() && sendOTP($email, $otp)) {
            $_SESSION['temp_user_id'] = $conn->insert_id; $_SESSION['temp_role'] = 1;
            $_SESSION['sys_msg'] = ['type' => 'success', 'title' => 'Thành công', 'msg' => 'Mã xác thực đã gửi về email.'];
            header("Location: verify_otp.php"); exit();
        } else {
            $sys_msg = ['type' => 'error', 'title' => 'Lỗi', 'msg' => 'Không thể đăng ký lúc này.'];
        }
    }
}
?>
...
<body style="display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 20px;">
    <div class="table-wrapper" style="width: 450px; margin: auto;">
        ...
        <form method="POST" action="">
            ...
            <div class="form-group"><label>Giới tính:</label><select name="gender"><option value="0">Nam</option><option value="1">Nữ</option><option value="2">Khác</option></select></div>
            <div class="form-group"><label>Địa chỉ:</label><textarea name="address" rows="2" required></textarea></div>
            <button type="submit" class="btn btn-success" style="width: 100%; margin-top: 10px;">Đăng ký & Nhận mã OTP</button>
        </form>
        <div style="text-align: center; margin-top: 15px;"><a href="login.php" style="color: var(--secondary-color); font-weight:bold;">Đã có tài khoản? Đăng nhập</a></div>
    </div>
