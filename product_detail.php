...
// ĐÃ SỬA LỖI: Thêm điều kiện is_hidden = 0 để người dùng không truy cập được vào sản phẩm đã ẩn bằng URL
$product = $conn->query("SELECT p.*, c.name as cat_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.id = $id AND p.is_hidden = 0")->fetch_assoc();
...
// Xác định ảnh chính
$main_img = !empty($product['image']) ? $product['image'] : 'https://via.placeholder.com/600x600?text=No+Image';

// LẤY DANH SÁCH ẢNH PHỤ TRỢ TỪ DATABASE
$extra_images = [];
$img_query = $conn->query("SELECT image_url FROM product_images WHERE product_id = $id");
...
// ĐÃ SỬA LỖI: Thêm điều kiện is_hidden = 0 để loại bỏ các sản phẩm đã xóa/ẩn khỏi danh sách gợi ý
$related = $conn->query("SELECT * FROM products WHERE category_id = {$product['category_id']} AND id != $id AND is_hidden = 0 ORDER BY RAND() LIMIT 4");
...
<!-- ẢNH LỚN -->
<img src="<?= $main_img ?>" id="mainProductImg" class="detail-main-img" alt="<?= htmlspecialchars($product['name']) ?>" style="transition: opacity 0.3s ease;">
<!-- THANH TRƯỢT ẢNH THU NHỎ (THUMBNAILS) -->
<div class="thumb-grid">
<!-- Ảnh đại diện làm ảnh đầu tiên -->
<img src="<?= $main_img ?>" class="thumb-item active" onclick="changeImage(this, '<?= $main_img ?>')" alt="Thumb">
<!-- Vòng lặp in các ảnh phụ trợ -->
<?php foreach($extra_images as $ex_img): ?>
<img src="<?= $ex_img ?>" class="thumb-item" onclick="changeImage(this, '<?= $ex_img ?>')" alt="Thumb">
<?php endforeach; ?>
</div>
</div>
...
<script>
// --- XỬ LÝ CLICK ĐỔI ẢNH MINH HỌA ---
function changeImage(element, src) {
    let mainImg = document.getElementById('mainProductImg');
    // Hiệu ứng mờ nhẹ khi chuyển ảnh
    mainImg.style.opacity = 0.6;
    setTimeout(() => {
        mainImg.src = src;
        mainImg.style.opacity = 1;
    }, 150);
    // Xóa class active ở mọi ảnh nhỏ
    let thumbs = document.getElementsByClassName('thumb-item');
    for (let i = 0; i < thumbs.length; i++) {
        thumbs[i].classList.remove('active');
    }
    // Thêm class active cho ảnh đang được click
...
function checkDetailStock() {
    ...
    if (variantsData.length > 0) { 
        infoSpan.innerText = maxStock > 0 ? `(Kho còn: ${maxStock})` : '(Hết hàng)'; 
    }
